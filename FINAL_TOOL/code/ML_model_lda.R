...
# GLOBAL VARIABLES
# Raw data folder with the idat files
IDAT_DIR <- "/home/user14/Nextcloud/share2/share/20210322_TFM_Eva/data/Eva_Samples_arrays/"
# Folder with the samplesheet
DATA_DIR <- "/home/user14/Nextcloud/share2/share/20210322_TFM_Eva/data/"
# Path where the temporary files and final b-values are stored
BVALS_DIR <- "/home/user14/Nextcloud/share2/share/20210322_TFM_Eva/data/Eva_Samples_b_vals/"
...
RESULTS_DIR <- "/home/user14/Nextcloud/share2/share/20210322_TFM_Eva/results/"
# Path where the code of the project is stored
CODE_DIR <- "/home/user14/Nextcloud/share2/share/20210322_TFM_Eva/code/"
...
SAMPLESHEET <- "samplesheet_eva.csv"
...
# export CSV file
write.csv(finalPredictions, file = file.path(RESULTS_DIR, "final_prediction_lda.csv"))

# export CSV file with an additional column with the predicted class
final_results_lda <- finalPredictions %>%
  mutate(Sample_Name = rownames(finalPredictions)) %>%
  mutate(max_val_lda = apply(finalPredictions, 1, max)) %>%
  mutate(class_lda = colnames(finalPredictions)[apply(finalPredictions, 1, which.max)])
write.csv(final_results_lda, file = file.path(RESULTS_DIR, "final_result_lda.csv"))
